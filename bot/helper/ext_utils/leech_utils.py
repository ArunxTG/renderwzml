...
async def format_filenamex(file_, user_id, dirpath=None, isMirror=False):
    user_dict = user_data.get(user_id, {})
    ftag, ctag = ('m', 'MIRROR') if isMirror else ('l', 'LEECH')
    prefix = user_dict.get('prefix', '@MS_LinkZzzz')
    #prefix = config_dict[f'{ctag}_FILENAME_PREFIX'] if (val:=user_dict.get(f'{ftag}prefix', '')) == '' else val
    remname = config_dict[f'{ctag}_FILENAME_REMNAME'] if (val:=user_dict.get(f'{ftag}remname', '')) == '' else val
    suffix = config_dict[f'{ctag}_FILENAME_SUFFIX'] if (val:=user_dict.get(f'{ftag}suffix', '')) == '' else val
    lcaption = config_dict['LEECH_FILENAME_CAPTION'] if (val:=user_dict.get('lcaption', '')) == '' else val
    ...
    cap_monox =  f"<{config_dict['CAP_FONT']}>{nfile_}</{config_dict['CAP_FONT']}>" if config_dict['CAP_FONT'] else nfile_
    if lcaption and dirpath and not isMirror:
        ...
        if len(slit) > 1:
            for rep in range(1, len(slit)):
                args = slit[rep].split(":")
                if len(args) == 3:
                    cap_monox = cap_monox.replace(args[0], args[1], int(args[2]))
                elif len(args) == 2:
                    cap_monox = cap_monox.replace(args[0], args[1])
                elif len(args) == 1:
                    cap_monox = cap_monox.replace(args[0], '')
        cap_monox = cap_monox.replace('%%', '|').replace('&%&', '{').replace('$%$', '}')
    return file_, cap_monox
